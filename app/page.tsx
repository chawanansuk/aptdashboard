...
const MaintLogView    = lazy(() => import("@/components/MaintLogView"));
  ...
  async function handleAddTask(values: import("@/lib/taskSchema").TaskFormValues) {
    ...
    try {
      ...
      if (data.ok && (data as { skipped?: string }).skipped === "duplicate-open") {
        ...
        // r29: เตือนสีเหลือง 10 วิ + ไม่ปิดฟอร์ม — เดิม toast ฟ้า 4 วิแล้วปิดฟอร์ม
        // ดูเหมือนบันทึกสำเร็จ ลูกน้องเพิ่งเจอ "กดเอาลูกค้าเข้าแล้วไม่เซฟ"
        toast.warning("ยังไม่ได้บันทึก — มีงานเดียวกันของห้องนี้วันนี้อยู่แล้ว", {
          description: "ถ้าเป็นลูกค้าคนละคน ใส่ชื่อ/เบอร์ให้ต่างจากงานเดิม หรือเปลี่ยนวันที่ แล้วกดบันทึกอีกครั้ง",
          duration: 10000,
        });
        refresh();
      } else if (data.ok) {
  ...
  return (
    ...
    >
      {isInitial && rooms.length === 0 && status !== "error" && <SkeletonLoader />}
          ...
          {activeView === "maintlog" && (
            <ErrorBoundary level="route" label="บันทึกซ่อมบำรุง">
