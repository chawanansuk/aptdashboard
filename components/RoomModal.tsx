...
// Lazy-load history tab: only fetches the chunk + makes the API call
// when the user actually clicks the tab
const RoomHistoryTab = lazy(() => import("./RoomHistoryTab"));
      ...
      <div className="ac-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <nav className="ac-modal-tabs" role="tablist" aria-label="แท็บข้อมูลห้อง">
          ...
          <button
            ...
            className={`ac-modal-tab ${tab === "history" ? "is-active" : ""}`}
        ...
        <div className="ac-modal-body">
              ...
              {room.pastTasks.length > 0 && (
                <>
                  ...
                  <button
                    ...
                    onClick={() => setShowLegacyHistory((v) => !v)}
                  >
                    <span>{showLegacyHistory ? "▾" : "▸"} ประวัติงาน ({room.pastTasks.length})</span>
                  </button>
                  {showLegacyHistory && (
          ...
          {tab === "history" && (
            <Suspense fallback={<div className="ac-empty" style={{ padding: 24, color: "#94A3B8" }}>กำลังโหลด...</div>}>
              <RoomHistoryTab building={room.building} room={room.room} />
            </Suspense>
          )}
        </div>
